                    ...
                    <template v-for="com in panel[1]">
                      ...
                      >
                        <div class="com-menu-item">
                          <span class="com-menu-name" v-html="com.highlights || com.name" :title="com.name"></span>
                          <DownloadOutlined
...
import { dataSearch, getPageWarehouse, savePageWarehouse } from "@/composition/index";
...
function useGetPageWarehouse() {
  refreshPageWarehouse();
  setTimeout(async () => {
    getPageWarehouseList();
  }, 200);
}
async function getPageWarehouseList() {
...
function importToDesigner(id: string) {
  Modal.confirm({
    ...
    onOk: async () => {
      getPageWarehouse(id, String(route.query.id || ""), String(route.query.emit || ""));
    },
  });
}
</script>
