...
export function getTrees() {
  ...
  const allTree = [...innerDropzoneTree, ...bottomcontentTree];
  return allTree;
}
...
export function getTreeData(element: HTMLElement) {
  ...
  if (children.length) {
    children.forEach((element: any) => {
      if (element.id && element.classList.contains("draggable2") && element?.componentModel?.model) {
 ...
 * 图层面板组件树选中
 */
export async function onSelect(keys: any, info: any) {
  // 被隐藏组件禁止选中
 ...
 * 重置图层面板tree数据
 */
export function useResetTree(changeTree = true, changeSelected = true, changeExpanded = true, changeChecked = true) {
  changeTree ? useSettingStore.setTreeData(getTrees()) : void 0;
  changeSelected ? useSettingStore.setSelectedKeys([]) : void 0;
  changeExpanded || changeChecked
    ? setTimeout(() => {
        changeCheckedAndExpanded(changeExpanded, changeChecked);
      }, 0)
    : void 0;
}
 ...
 * @param e
 */
export async function breadcrumbClick(e: any) {
