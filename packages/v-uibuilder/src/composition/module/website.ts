import { treeDataOptions, websiteOptions } from "@/types/website";
import { contentTypeEnum, navigationEnum, openModeEnum } from "@/enums/websiteEnum";
import { createHash } from "@/utils/uuid";
import { saveWebsite } from "@/api/uibuilder/website";
 ...
 * 默认数据
 */
const website: websiteOptions = {
  projectId: "",
  navigation: navigationEnum.DUAL,
  theme: "dark",
  logo: "",
  mainTitle: "未命名站点",
 ...
 * @param id
 */
export function createWebsite(id: string) {
  website.projectId = id;
  saveWebsite({ websiteName: website.mainTitle, config: website })
    .then((res) => {
      const {
        results,
        info: { msg = "" },
      ...
      if (msg !== "success") {
        message.destroy();
        message.error("新建站点失败!");
        return;
      }
      router.push(`/uibuilder/website?id=${results.id}`);
    })
    .catch((error) => {
      message.error("新建站点失败!");
    });
...
export function handleUrl(info: treeDataOptions) {
  ...
  switch (info.contentType) {
    case contentTypeEnum.PAGE:
      url = `/#/uibuilder/publish/${info.pageId}?id=${info.pageId}`;
      break;
