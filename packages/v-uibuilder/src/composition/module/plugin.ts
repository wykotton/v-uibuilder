...
export function getComponents() {
  ...
  const components = []; //.concat(...allComponent);
 ...
 * @returns
 */
export async function checkComponentDefined(componentName: string, componentsList: any[]) {
  const target = componentsList.find((current) => current.componentName === componentName);
  if (target) {
  ...
  try {
    await DesignerPage.whenDefinedComponent(componentName)
      ...
      .catch(() => {
        console.error(`${componentName}元件未注册或元件未定义!`);
      });
  } catch (error) {
    console.error(`${componentName}元件注册失败!请检查元件仓库是否加载成功:`, error, target);
    return false;
  }
