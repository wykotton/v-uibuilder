...
import { stringifyDeep } from "../index";
...
class DesignerPage {
  ...
  setComponentsPageModel(componentModel: ISchema) {
    const index = this.componentsArray.findIndex((item) => item.id === componentModel.id);
    if (index !== -1) {
      this.componentsArray[index] = componentModel;
    } else {
  ...
  setExceptionComponents(componentModel: ISchema) {
    const index = this.exceptionComponents.findIndex((item) => item.id === componentModel.id);
    if (index !== -1) {
      this.exceptionComponents[index] = componentModel;
    } else {
  ...
  deleteExceptionComponents(id: string) {
    this.exceptionComponents = this.exceptionComponents.filter((item) => item.id !== id);
  }
}
