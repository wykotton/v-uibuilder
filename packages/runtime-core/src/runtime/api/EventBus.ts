import { IEventBus, IMessage, IRouterConfig, IRouterInfo, IReceive } from "@zzjz/v-uibuilder-types";
...
import { PagecastEventBusPlugin } from "../widgets/pagecast/PagecastEventBusPlugin";
type CachedFunction = {
  params: any;
  fn: Function;
};
...
class CacheHandler {
  private cachedFunctions: Map<string, CachedFunction[]> = new Map();

  createCachedFunction(eventType: string, fn: Function, params: any): CachedFunction[] {
    if (!this.cachedFunctions.has(eventType)) {
      this.cachedFunctions.set(eventType, []);
    }
    const newCachedFn = { fn, params };
    this.cachedFunctions.get(eventType)?.push(newCachedFn);
  ...
  getAllCachedFunctions(eventType: string): CachedFunction[] {
    return this.cachedFunctions.get(eventType) ?? [];
  }
}
...
class EventBus implements IEventBus {
  ...
  parentEventBusInit = () => {
    ...
    this.registerEventHandlers(PARENT_EVENTBUS_NAME, (data: any) => {
      if (!this.pageLoaded) {
        cacheHandler.createCachedFunction(PARENT_EVENTBUS_NAME, this.eventMap[PARENT_EVENTBUS_NAME], data);
        return;
      }
      cacheHandler.getAllCachedFunctions(PARENT_EVENTBUS_NAME)?.forEach(({ fn, params }) => {
        fn && fn(params);
      });
  ...
  eventBusInit = () => {
    ...
    this.registerEventHandlers(EVENTBUS_NAME, (data: any) => {
      if (!this.pageLoaded) {
        cacheHandler.createCachedFunction(EVENTBUS_NAME, this.eventMap[EVENTBUS_NAME], data);
        return;
      }
      cacheHandler.getAllCachedFunctions(EVENTBUS_NAME)?.forEach(({ fn, params }) => {
        fn && fn(params);
      });
