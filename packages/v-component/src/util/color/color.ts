...
function standardizeColor(name: string) {
	...
	(ctx as any).fillStyle = name;
	return (ctx as any).fillStyle === "#000000" ? null : (ctx as any).fillStyle;
}
...
export function hexToHsv(hex: string) {
	const hsv = (hex as any).match(/.{2}/g).map((v: any) => parseInt(v, 16));
	return rgbToHsv(hsv[0], hsv[1], hsv[2]);
}
...
export function parseToHSVA(str : string) {
	 ...
	 * @return {*}
	 */
	const numarize = (array: any) => array.map((v: any) => (/^(|\d+)\.\d+|\d+$/.test(v) ? Number(v) : undefined));
