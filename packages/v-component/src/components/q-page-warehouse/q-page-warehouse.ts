    ...
    const component = defineComponent({
      template: `
            <a-config-provider :locale="zhCN">
              <a-modal v-model:visible="importVisible" v-if="importVisible" title="导入页面" :maskClosable="false" width="80%" 
                  :bodyStyle="{ maxHeight: '700px', overflow: 'auto' }" :closable="false" :confirmLoading="confirmLoading"
              ...
              >
                ...
                <a-spin :spinning="spinning" tip="Loading...">
                  <div>
                    ...
                    <div v-if="pageList.length" style="width: 100%;height: 400px;margin-top: 20px;overflow-y: auto;">
                      <a-checkbox-group v-model:value="listValue" style="width: 100%">
                        <a-row>
                          <a-col v-for="item in pageList" :span="4">
                            <a-checkbox :value="item.id" style="margin-top: 10px">{{item.name}}</a-checkbox>
                          </a-col>
                        </a-row>
                      </a-checkbox-group>
                    </div>
      ...
      setup() {
        ...
        function handleOk() {
          ...
          try {
            const pageList = JSON.parse(jsonStr);
            if (checkComponentList(pageList)) return;
            checkCombination(pageList);
          } catch (error) {
        ...
        // 校验数据
        function checkComponentList(pageList: any) {
          if (!isArray(pageList)) {
          ...
          pageList.forEach((item) => {
            if (!item.custom_model || !item.name || !item.type || !item.group) isError = true;
          });
          if (isError) {
        ...
        // 检查数据是否重复
        // @ts-ignore
        async function checkCombination(pageList: any) {
          ...
          const config = {
            url: "/v-uibuilder-server/ui-builder/search-page-warehouse",
            method: "GET",
            headers: {
          ...
          const getInfo = async (config: any) => {
            await axios(config)
              .then((res) => {
                ...
                if (msg === "success") {
                  results[0] ? repeatInfo.push(results[0].name) : void 0;
                } else {
                  requestError = true;
                }
              })
          ...
          if (repeatInfo.length) {
            Modal.confirm({
              ...
              content: createVNode(
                "div",
                { style: "color:red;" },
                `${repeatInfo.join(", ")} 等${repeatInfo.length}个页面已存在`
              ),
              onOk() {
                importCombination(pageList);
              },
            });
          } else {
            importCombination(pageList);
        ...
        // 导入数据
        async function importCombination(pageList: any) {
          const token = getUserToken();
          const config = {
            url: "/v-uibuilder-server/ui-builder/save-page-warehouse",
            method: "POST",
            headers: {
              Authorization: "Bearer " + token,
            },
          ...
          if (radioType.value === "user") {
            ...
            config.data.data = pageList.map((item: any) => {
              item.group = ["我的页面"];
              return item;
            });
          } else {
          ...
          await axios(config)
            .then((res) => {
              ...
              if (msg === "success") {
                message.success("导入页面成功");
                // 刷新元件仓库
                initCombination();
              } else {
                message.error("导入页面失败!");
                console.log(msg);
              }
            })
            .catch((err) => {
        ...
        function showExportDeleteModal(type: string) {
          ...
          type === "export" ? (exportDeleteTitle.value = "导出页面") : (exportDeleteTitle.value = "删除页面");
          radioType.value = "user";
        ...
        // 获取页面列表
        // @ts-ignore
        async function getPageWarehouse() {
          const token = getUserToken();
          const userId = getUserId();
          if (radioType.value === "user" && (!token || !userId)) {
            message.error("未找到用户信息, 无法获取页面数据!");
            return false;
          }
          spinning.value = true;
          const config = {
            url: "/v-uibuilder-server/ui-builder/get-page-warehouse",
            method: "GET",
            headers: {
              Authorization: "Bearer " + token,
            },
         ...
         */
        function selectAll() {
          listValue.value = pageList.value.map((item) => {
            return item.id;
          });
        }
        ...
        async function exportPage() {
          ...
          const config = {
            url: "/v-uibuilder-server/ui-builder/find-page-warehouse",
            method: "GET",
            headers: {
              Authorization: "Bearer " + token,
            },
          ...
          await axios(config)
            .then((res) => {
              ...
              const fileName = `${firstName || "page-warehouse"}...等(共${length}个页面)_${getFormatTime()}`;
              if (msg === "success") {
        ...
        async function deletePage() {
          ...
          const config = {
            url: "/v-uibuilder-server/ui-builder/delete-page-warehouse",
            method: "POST",
            headers: {
              Authorization: "Bearer " + token,
            },
        ...
        async function initCombination() {
          ...
          const userId = getUserId();
          if (!token) return;
          let userPageWarehouse = [] as any;
          let publicPageWarehouse = [] as any;
          const config = {
            url: "/v-uibuilder-server/ui-builder/get-page-warehouse",
            method: "GET",
            headers: {
          ...
          await axios(config).then((res) => {
            ...
            if (msg === "success") {
              publicPageWarehouse = results;
            }
          });
          const temp = [...userPageWarehouse, ...publicPageWarehouse];
        ...
        onMounted(() => {
          ...
          initCombination();
          // 监听CustomEvent
          window.addEventListener("pageWarehouse", (e: any) => {
...
declare global {
  interface HTMLElementTagNameMap {
    "q-page-warehouse": QPageWarehouse;
  }
}
